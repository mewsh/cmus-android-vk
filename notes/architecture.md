...
- patches/cmus (14; the four upstream-candidate fixes lead the stack so
  ...
  const. 0004 (upstream candidate, un-gated, from pgaskin/cmus
  `fix-utf8-invalid-overflow`, cmus/cmus#1520) makes u_print_size() count 4
  bytes for exactly what u_set_char() prints as `<XX>` — control byte 0x1f
  and any uchar above 0x10ffff, which is every invalid input byte since
  u_get_char() returns `byte | U_INVALID_MASK` for those and u_char_size()
  sized that at 1 — so u_str_print_size() no longer undersizes the buffers
  gbuf_add_ustr() (every formatted UI line), mpris.c and android.c's
  android_json_str() allocate from it (heap/stack overflow on non-UTF-8
  strings). 0005 adds the app IPC socket (android.c/android.h + hook
  ...
  *trylocks* the player mutexes, falling back to the whole-second
  ...
  still builds with the upstream Makefile), 0006 removes remote-stream
  ...
  0007 (upstream candidate, un-gated) makes do_cmus_save atomic —
  ...
  would clobber a playlist named X.tmp), 0008 (Android-only, gated on
  ...
  the upstream Makefile is unaffected: 0009 drops the built-in cmus-remote
  ...
  the app has its own IPC socket; 0010 routes debug output to logcat (tag
  ...
  error level always; 0011 pins the file browser's default dir: browser_init()
  ...
  folder, or the storage root when all-files access is granted); 0012 drops
  ...
  build no longer compiles it); 0013 drops CD-audio (no cdio plugin is built,
  ...
  0014 makes option_set accept-and-ignore the specific removed option names
...
- `native/cmus/`: cmus core (the Makefile's cmus-y set, mpris off, and
  http.c/server.c/spawn.c/discid.c out — server by 0009, the last two by
  0012/0013 which strip their callers, + the patch's android.c;
  `CONFIG_ANDROID` set as a plain compile definition on the core only; links
  `-llog` for the logcat debug output of patch 0010) as an
  ...
  (patch 0008 / CONFIG_STATIC_PLUGINS) as per-plugin OBJECT libs whose
...
- `CmusService` — mediaPlayback foreground service owning the cmus
  ...
  plugin .so files — patch 0008 builds them into the binary) +
  ...
  is on, read once at startup by patch 0010 so a change needs a respawn, +
  ...
  shared Music folder, pinned as the browser default every launch by 0011;
...
pending: 0001 (aaudio remap buffer size), 0007 (atomic playlist saves);
0002/0003/0004 (wavpack .wvc leak, opus callbacks/fdsan, invalid-UTF-8
print size = cmus/cmus#1520) are on Patrick's fork as `fix-wavpack-fd-leak`
/ `fix-opus` / `fix-utf8-invalid-overflow` — pull from there if they change
